...
async def help(ctx, x=0, y=0, zoom = 1000, size =10):
    ...
    for i in client.commands:
         if i.help:embed.add_field(name = f'-**{str(i.name)}**-' + str((f'({aliase})' for aliase in i.aliases) if i.aliases else ''), value=i.help,inline=True)#command objects are genrators so you have to parse to str
    await ctx.reply(embed=embed)
